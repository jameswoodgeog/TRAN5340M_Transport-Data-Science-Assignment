...
\newpage
```
As we can see in @fig-circ_dist the histogram shows that distribution displays a small right-skew. The boxplot allows us to see that

equating to a somewhat low circuity overall in York.
...
To try to better understand why these two OAs routes result in circuity calculations that are so different, plots were created to visualise the routes that make up those calculations. @fig-hlroutes clearly shows that the OA with the lowest circuity is in the village of Haxby to the north of York. This circuity implies that the structure of the village is such that services are centered around the high street, which itself consists of a single fairly straight road through the village. By contrast, the OA with the highest circuity was to the north-west of York and is overall more rural, and is less densely built up. The location of the service is important to the calculations here, because it sits away from the main street through the village of Stockton on the Forest. This means that the route to the service is longer and more circuitous than it would be if the service was located on the main street.

As can be seen in the plot for the lowest circuity, there seems to be a larger distance to be covered to reach the services. The plot below shows the different walking distances for each route for these two exxamples.

### PLOT WALKING DISTANCES

rural locateions have higher car use and ownership, so planning should put services in the most accesible place
