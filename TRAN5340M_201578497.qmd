...
from_to <- hcare_all[,c("index", "oa" , "from", "to")]
...
from_to$circuity <- ((hcare_all$walkDistance - from_to$e)/hcare_all$walkDistance)
saveRDS(from_to, "Data/from_to.RDS")
```
...
```{r}
# Plot the oa_circuity filling the polygons with graduated coolor scheme from the circuity column
tm_shape(oa_circuity) +
  tm_fill("mean_circuity", palette = "viridis") +
  tm_basemap(server = "OpenStreetMap")

ggplot() +
  geom_sf(data = oa_circuity, aes(fill = mean_circuity)) +
...
# Plot the polygons and add a basemap
mapview(oa_circuity, zcol = "mean_circuity")
